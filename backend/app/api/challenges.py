...
async def challenge(session_id: str, body: ChallengeInput):
    ...
    if session.completed:
        raise HTTPException(409, "Presentation is complete")
    segment = session.segments[session.current_segment_index]
    if body.segment_id != segment.id:
        raise HTTPException(409, "Challenge is not for the current segment")
    if any(c.segment_id == segment.id for c in session.challenges):
        raise HTTPException(409, "This segment has already been challenged")
    relevant = [error for error in session.errors if error.id in segment.error_ids]
    try:
        evaluation = await get_provider().evaluate_challenge(
            segment, body.reason, relevant
        )
    ...
    session.challenges.append(
        Challenge(
            ...
            identified_error_id=identified,
            reasoning_score=evaluation["reasoning_score"],
            feedback=evaluation["feedback"],
        )
    )
    return {
        key: evaluation[key] for key in ("valid", "reasoning_score", "feedback")
    }
